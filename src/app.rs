...
impl cosmic::Application for AppModel {
    ...
    const APP_ID: &'static str = "com.github.luthfynur.battery-percentage-applet";
    ...
    fn subscription(&self) -> Subscription<Self::Message> {
        ...
        Subscription::batch(vec![
            ...
            iced::time::every(Duration::from_secs(1)).map(|_| Message::UpdateBatteryPercentage),
        ])
    }
    ...
    fn update(&mut self, message: Self::Message) -> Task<cosmic::Action<Self::Message>> {
        match message {
            ...
            Message::UpdateBatteryPercentage => {
                let new_battery_percentage = get_battery_percentage();
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        let text = self.core.applet.text(&self.battery_percentage).size(15.0);
    ...
    if output.status.success() {
        ...
        for line in stdout.lines() {
            if line.trim_start().starts_with("percentage:") {
                return format!("{}",line.replace("percentage:", "").trim());
            }
        }
    } else {
